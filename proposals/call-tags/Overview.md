...
These can be *multiple* canonical call tags, and they can include custom call tags either generated by (and specific to) this module instance or imported from other module instances.
...
[Efficient implementation of Java interfaces: Invokeinterface considered harmless](https://dl.acm.org/doi/10.1145/504282.504291) used this to implement Java interface methods and found the technique to make interface-method dispatch typically as fast as class-method dispatch, and our recent experiments confirmed that this behavior still holds in modern hardware.
...
[Transitioning from Structural to Nominal Code with Efficient Gradual Typing](https://www.cs.cornell.edu/~ross/publications/monnom/monnom-oopsla21.pdf) used this to implement a dynamically typed language and found it to work quite well.
...
[Transitioning from Structural to Nominal Code with Efficient Gradual Typing](https://www.cs.cornell.edu/~ross/publications/monnom/monnom-oopsla21.pdf) used roughly this approach to implement efficient interop between a Java-like language and a JavaScript-like language.
...
* `call_tag.canon $functype` derives the canonical call tag of type `$functype` from the function type `$functype : [ti*] -> [to*]`, where all of `ti*` and `to*` are "concrete" (discussed [below](#asbstraction))
...
When defining a `func` of type `[ti*] -> [to*]`, one can optionally specify `(call_tag $calltag*)`, where each `$calltag`'s type must be a supertype of with `[ti*] -> [to*]`.
If so, the `funcref` returned by `func.ref` for this `func` handles exactly the call tags in `$calltag*`.

`func_switch ($call_tag $func)* $func_switch` defines a new function, where the type of each `$call_tag` is a supertype of its corresponding `$func`'s type'.
The new function has no type and cannot be directly called.
However, the `funcref` returned by `func.ref` for this `func_switch` handles each `$call_tag` by calling the corresponding `$func`.
If a call tag is not recognized, then if `$func_switch` is specified the call tag and arguments are forwarded to it, otherwise the fall-back handler of the call tag is tail-called with the arguments. 

### Instructions

`call_with_tag $call_tag : [funcref ti*] -> [to*]`, where `$call_tag : [ti*] -> [to*]`, calls the given `funcref` with the specified call tag using the values on the stack as the arguments.

`call_return_with_tag $call_tag : [funcref ti*] -> [to*]`, where `$call_tag : [ti*] -> [to*]`, tail calls the given `funcref` with the specified call tag using the values on the stack as the arguments, where `[to*]` is a subtype of the result type of the function containing this instruction.
...
In light of these issues with interface tables, [Efficient implementation of Java interfaces: Invokeinterface considered harmless](https://dl.acm.org/doi/10.1145/504282.504291) developed a different implementation strategy, which they called interface-method tables.
...
This poses a type-checking problem for wasm: for `Sub`'s refined v-table's type to be a subtype of `Super`'s v-table's type, one needs structural subtyping of typed function references, which unfortunately [is undecidable with even modest extensions](https://dl.acm.org/doi/10.1145/143165.143228).
...
Then, rather `Sub` using a single `func` to implement both call tags for `bar`, we have `Sub` use a `func_switch`.
For `$Super.bar`, it simply switches to `Sub`'s implementation of `bar`.
For `$Sub.bar`, it switches to a function that boxes the given `i32` and then tail calls `Sub`'s implementation of `bar`.
...
[Transitioning from Structural to Nominal Code with Efficient Gradual Typing](https://www.cs.cornell.edu/~ross/publications/monnom/monnom-oopsla21.pdf) implemented this feature by using call tags.
...
[Transitioning from Structural to Nominal Code with Efficient Gradual Typing](https://www.cs.cornell.edu/~ross/publications/monnom/monnom-oopsla21.pdf) also took the above technique a bit further to make it so that untyped objects could dynamically acquire nominal interfaces in order to provide interop between its JavaScript-like subset and its Java-like subset.
